...
class AdminBookingRequestFileController extends Controller
{
    use StreamsBookingRequestFiles;

    public function flightDocument(FlightBookingRequest $flightBookingRequest, string $document): StreamedResponse
    {
        $path = match ($document) {
    ...
    public function hotelDocument(HotelBookingRequest $hotelBookingRequest, string $document): StreamedResponse
    {
        $path = match ($document) {
            'voucher' => $hotelBookingRequest->voucher_path,
            'invoice' => $hotelBookingRequest->invoice_path,
            'transfer_voucher' => $hotelBookingRequest->transfer_voucher_path,
            default => null,
        };
    ...
    public function cruiseDocument(CruiseBookingRequest $cruiseBookingRequest, string $document): StreamedResponse
    {
        $path = match ($document) {
    ...
    public function carDocument(CarBookingRequest $carBookingRequest, string $document): StreamedResponse
    {
        $path = match ($document) {
    ...
    public function insuranceDocument(InsuranceBookingRequest $insuranceBookingRequest, string $document): StreamedResponse
    {
        $path = match ($document) {
